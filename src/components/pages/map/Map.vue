  ...
  <div class="container">
      <l-map style="height: 700px;z-index: 1" :zoom="zoom" :center="center" :options="{zoomControl: false}">
        ...
        <l-marker v-for="mark in datacollection.Polyclinics.data" :lat-lng="[mark.lat, mark.long]" :key="mark.title">
          <l-icon>
          <img :src="datacollection.Polyclinics.img" style="height:40px;width:40px">
          </l-icon>
          <l-tooltip style="text-align: left">{{ mark.name }}<br>Tel: {{ mark.tel }}<br>Hours: {{ mark.hours }}<br>Address: {{ mark.address }}</l-tooltip>
        </l-marker>
        </div>
        ...
        <div v-if="datacollection.SASH_for_Children.show">
          <l-marker v-for="mark in datacollection.SASH_for_Children.data" :lat-lng="[mark.lat, mark.long]" :key="mark.title">
            <l-icon>
              <img :src="datacollection.SASH_for_Children.img" style="height:40px;width:40px">
            </l-icon>
            <l-tooltip style="text-align: left">{{ mark.name }}<br>Tel: {{ mark.tel }}<br>Hours: {{ mark.hours }}<br>Address: {{ mark.address }}</l-tooltip>
          </l-marker>
        </div>
      ...
      <v-select multiple :options="icons" label="title" @input="setSelected" class="inner">
        <template slot="option" slot-scope="option">
          <img :src="option.cardImage" style="height:40px;width:40px"/>
...
import { LMap, LTileLayer, LMarker, LIcon, LTooltip, LControlZoom } from 'vue2-leaflet';
